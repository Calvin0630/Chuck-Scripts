SinOsc a  =>Gain g=> Chorus chorus => dac;
SinOsc b => g;
SinOsc c => g;
0 => g.gain;
...
.2 =>chorus.mix;
Std.mtof(rootNote) =>chorus.modFreq;
rootNote+24 =>rootNote;
while (true) {
    
    [0,2,4,7,9,12] @=> int scale[];
    0=>int x;
    while(true) {
        if(x>scale.cap()-2)0=>x;
        if(x%2==0) {
            repeat(3) {
                playTriple([rootNote+scale[0],rootNote+scale[2],rootNote+scale[4]],3*beat/4);
                wait(beat/4);
            }
            
        }
        else {
            playTriple([rootNote+scale[1],rootNote+scale[3],rootNote+scale[5]], 3*beat/2);
            wait(beat/2);
